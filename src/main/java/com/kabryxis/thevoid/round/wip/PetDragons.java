...
public class PetDragons extends AbstractRound {
	...
	/*public class PetDragon extends EntityInsentient implements IComplex, IMonster {
		
		static {
			String name = "EnderDragon";
			Class<?> clazz = PetDragon.class;
		...
		public void setTarget(Gamer target) {
			this.target = target;
		}
		...
		public PetDragon(Location loc, Gamer owner, Vector center) {
		...
		private void attackTarget() {
			if(currentTarget == null && target != null) {
				if(!target.isAlive()) {
					owner.message(target.getName() + " has been killed, your dragon needs a new target!");
					target = null;
				}
		...
		/**
		 * Destroys the blocks the Dragon flies through.
		 * 
		 * @param axisalignedbb The BoundingBox of the Dragon, to know what blocks are being touched.
